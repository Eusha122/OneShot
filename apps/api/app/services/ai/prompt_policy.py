...
FORMATTING_RULES = (
    ...
    "- NEVER create ASCII diagrams or art. Use a mermaid diagram instead (see DIAGRAM RULES below).\n"
    ...
    "DIAGRAM RULES:\n"
    "When the explanation benefits from a visual structure — processes, cycles, classifications, "
    "comparisons, hierarchies, cause-and-effect chains, flowcharts — output a mermaid diagram.\n"
    "Use ```mermaid fenced blocks. Supported diagram types:\n"
    "  - flowchart TD / LR  (processes, decision trees, step-by-step flows)\n"
    "  - graph TD / LR      (concept maps, classification trees)\n"
    "  - sequenceDiagram    (interactions between components or steps over time)\n"
    "  - classDiagram       (taxonomies, biological classifications, OOP hierarchies)\n"
    "  - stateDiagram-v2    (state transitions, life cycles)\n"
    "  - pie                (proportions, distributions)\n"
    "  - timeline           (historical events, chronology)\n"
    "Diagram rules:\n"
    "  - Node labels must be short (≤ 6 words). Put detail in the text explanation, not node labels.\n"
    "  - Do NOT wrap node text in quotes unless it contains special characters.\n"
    "  - Use --> for arrows, -- label --> for labelled arrows.\n"
    "  - Always include a diagram title as a comment: %% Title here\n"
    "  - Output ONE diagram maximum per response.\n"
    "  - Do NOT output a diagram for simple single-concept questions or pure math/formula questions.\n"
    "  - Example flowchart:\n"
    "```mermaid\n"
    "flowchart TD\n"
